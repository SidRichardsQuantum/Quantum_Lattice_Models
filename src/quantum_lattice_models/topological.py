...
def harper_hofstadter_square_lattice(
    ...
    for row in range(n_rows):
        for col in range(n_cols):
            ...
            if n_row is not None and n_col is not None:
                add_symmetric_hopping(
                    matrix, site, square_lattice_index_raw(n_row, n_col, n_cols), -hopping * phase
                )
...
def harper_hofstadter_square_lattice_sparse(
    ...
    dense = harper_hofstadter_square_lattice(
        n_rows,
        n_cols,
        hopping=hopping,
        flux=flux,
...
def haldane_honeycomb_lattice(
    ...
    validate_positive_int(n_rows, "n_rows")
    validate_positive_int(n_cols, "n_cols")
    matrix = np.zeros((2 * n_rows * n_cols, 2 * n_rows * n_cols), dtype=complex)
    for row in range(n_rows):
        for col in range(n_cols):
            ...
            for n_row, n_col in (
                ...
                if n_row is not None and n_col is not None:
                    add_symmetric_hopping(
                        matrix, a, honeycomb_index_raw(n_row, n_col, 1, n_cols), -t1
                    )
...
def haldane_honeycomb_lattice_sparse(
    ...
    dense = haldane_honeycomb_lattice(
        n_rows,
        n_cols,
        t1=t1,
        t2=t2,
