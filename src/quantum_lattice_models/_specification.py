"""Internal implementation for portable model specifications."""
...
class ModelSpec:
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        if self.symmetry_actions:
            result["symmetry_actions"] = [action.to_dict() for action in self.symmetry_actions]
        return result
        ...
        spec = cls(
            ...
            symmetry_actions=tuple(
                ...
                for record in _record_list(
                    data.get("symmetry_actions", []), "model.symmetry_actions"
                )
            ),
    ...
    def summary(self) -> dict[str, object]:
        ...
        summary: dict[str, object] = {
            ...
            "symmetry_action_count": len(self.symmetry_actions),
...
def create_model_spec(
    ...
    *,
    ...
    symmetry_actions: tuple[SymmetryAction, ...] | None = None,
    ...
    inferred_symmetry_actions = _infer_symmetry_actions(
        family,
        resolved,
        inferred_degrees,
    )
    spec = ModelSpec(
        ...
        symmetry_actions=(
            inferred_symmetry_actions if symmetry_actions is None else tuple(symmetry_actions)
        ),
...
def _spin_chain_physical_system(
    ...
    if family == "transverse_field_ising_parity_sector":
...
def _infer_symmetry_actions(
    family: str,
    parameters: dict[str, object],
    local_degrees: tuple[LocalDegreeOfFreedom, ...],
) -> tuple[SymmetryAction, ...]:
    """Infer exact discrete symmetry actions advertised by built-in models."""
    ...
    if base in {
        ...
        return (
            SymmetryAction(
                ...
                kind="onsite_product",
                degrees=tuple(degree.index for degree in local_degrees),
                operators=tuple("X" for _ in local_degrees),
                eigenvalues=(-1, 1),
                convention="P = X_0 X_1 ... X_(N-1)",
                metadata={
                    "conserved": True,
                    "sector_parameter": (
                        "parity" if base == "transverse_field_ising_parity_sector" else None
                    ),
                    "selected_eigenvalue": parameters.get("parity"),
                },
            ),
        )
    return ()
